                ...
                return (
                    <Link href={`/anime/${anime.mal_id}`} key={anime.mal_id} className="cursor-pointer md:m-3 m-2 border-2 border-color-dark-4 text-color-primary overflow-hidden relative">
                        <div className="md:px-3 md:pt-3 pt-2 px-2 hover:text-color-secondary transition-all relative block">
                            ...
                            <div className="w-full md:h-[50px] sm:h-[45px] h-[40px] overflow-hidden pt-2">
                                <h3 className="md:text-[14px] sm:text-[12px] text-[10px] text-bold">{anime.title}</h3>
                            </div>
                        </div>
                        <span className="flex justify-start items-center gap-1 absolute md:top-[220px] sm:top-[145px] top-[67px] md:ml-3 ml-2 pt-2 ">
                            {anime.genres ? anime.genres.slice(0,3).map((genre, index) => <h6 key={index} className="md:text-[7px] text-[5px] bg-color-red md:p-[3px] p-[2px] rounded-md">
                                {genre.name}</h6>) : null}
                        </span>
                        {anime.episodes ?
